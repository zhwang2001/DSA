...
"""
Explanation:
goechl is to take first and last target index within array
...
class Solution:
    def lengthOfLonfgestSubstring(self, s: str) -> int:
        ...
        for i in set(s):
        self.next = s[i + 1]
        if self.next in store_list 
            store_list.append([:s.index(i) + 1]
            s = s[s.index(i):]

        candidates = []
        find max length iteration in a list
        for i in range(store_list):
            candidates.append(len(i)
        return int(max(max_iteration))
